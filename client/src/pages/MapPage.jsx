...
export default function MapPage() {
  ...
  const { bases, attacks, tradePods, playerBases, playerBaseIds, visRadius, setMapData, removeAttack } = useMapStore();
  ...
  useEffect(() => {
    if (!socket) return;
    socket.on('map:attack_resolved', ({ attackId }) => removeAttack(attackId));
    socket.on('map:attack_launched', load);
    return () => {
      socket.off('map:attack_resolved');
  ...
  return (
    <div className="page flex flex-col" style={{ paddingBottom: 0 }}>
      ...
      {/* Selected base panel */}
      {selectedBase && !showAttack && (
        <div className="bg-space-800 border-t border-space-600/50 px-4 py-3 flex-shrink-0">
          <div className="flex items-center justify-between mb-2">
            <div>
              <div className="text-white font-medium text-sm">{selectedBase.name}</div>
